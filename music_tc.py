    ...
    async def _join(self, ctx: commands.Context):
        ...
        if ctx.voice_client:
            if ctx.voice_client.channel != ctx.author.voice.channel:
                ...
                await self.respond(ctx,embed=discord.Embed(title=":x: 機器人已經在你的語音頻道！",color=0xff0000), reply=True)
                return False
    ...
    async def _summon(self, ctx: commands.Context, *, channel=None):
        ...
        try:
            ...
            try:
                ...
                return await self.respond(ctx, embed=discord.Embed(title=":x: 找不到該頻道！", color=0xff0000), reply=True)
